...
func _draw() -> void:
	...
	# Handle the Background Grid Shader
	...
	for entity_id in SceneInstances.entity_manager.render_components:
		...
		# If the bullet is in parry range, override its color to a searing, overdriven HDR white
		elif render_data.is_parry_highlighted:
			core_color = Color(5.0, 5.0, 5.0, 1.0) 
		...
		for w_id in weapons:
			if w_id in SceneInstances.entity_manager.transform_components:
				...
				if weapon.is_aiming:
					...
					var laser_color = Color(1.0, 0.0, 0.0, 0.1 + (0.5 * charge_ratio))
